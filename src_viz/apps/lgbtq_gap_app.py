...
def sexual_orientation_types_stacked_bar(sexualorientation, content, absolute_relative, langs):
    ...
    df = df.rename(columns={"Sexual Orientation":"Sexual orientation", "set1": "Wiki", "abs_value": "Biographies", "rel_value":"% Biographies", "set2descriptor":"Qitem"}).dropna()
    ...
    fig = px.bar(df, x="Language", y=y, color="Sexual orientation", title="Types of sexual orientation", hover_data=['Wiki','Sexual orientation','% Biographies','Biographies','Qitem'], text='Biographies')
