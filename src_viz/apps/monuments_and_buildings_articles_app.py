...
### DASH APP ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### 
app = Dash(__name__,server = app_dev, url_base_pathname= webtype + '/monuments_and_buildings_articles/', external_stylesheets=external_stylesheets, external_scripts=external_scripts)

app.config['suppress_callback_exceptions']=True
title = "Monuments and buildings Articles"
...
topic_dict={'All':'all','Keywords':'keywords','Geolocated':'geolocated','Folk':'folk','Earth':'earth','Music Creations and Organizations':'music_creations_and_organizations','Sports and Teams':'sport_and_teams','Food':'food','Paintings':'paintings','GLAM':'glam','Books':'books','Clothing and Fashion':'clothing_and_fashion','Industry':'industry'}

target_langs_dict = language_names

features_dict = {'Editors':'num_editors','Edits':'num_edits','Pageviews':'num_pageviews','Inlinks':'num_inlinks','References':'num_references','Bytes':'num_bytes','Outlinks':'num_outlinks','Interwiki':'num_interwiki','WDProperties':'num_wdproperty','Discussions':'num_discussions','Creation Date':'date_created','Inlinks from CCC':'num_inlinks_from_CCC','Outlinks to CCC':'num_outlinks_to_CCC','Monument or building':'monuments_and_buildings'}
...
text_default = '''In this page you can retrieve articles about Monuments and Buildings from Wikipedia language editions, including all kinds of topics, and check its availability in a specific Wikipedia.'''
...
interface_row1 = html.Div([
    ...
    dbc.Tooltip(
        html.P("Select the language editions from which you want to retrieve the Monuments and Buildings articles they have in common. You can retrieve a single language if you want. The relevance features will be according to the first language.",
...
interface_row2 = html.Div([
    ...
    dbc.Tooltip(
        html.P(
            "Select a feature to sort the results (by default it uses the Qitem of the category of Monument or building it belongs to).",
...
#        query += 'REPLACE(r.page_title,"_"," ") as r.page_title, '
        ...
        query += 'WHERE r.monuments_and_buildings IS NOT NULL '
        ...
        if order_by == "none" or order_by == "None":
            query += 'ORDER BY r.monuments_and_buildings DESC '
...
#        df = pd.read_csv(databases_path + 'lgbt.csv')
        ...
        if order_by == "none" or order_by == "None":
            df = df.sort_values(by='monuments_and_buildings', ascending=False)
        else:
        ...
        main_title = 'Monuments and Buildings articles retrieved from '+source_language+' Wikipedia and its coverage by the target languages'


        text_results = '''
        The following table shows the resulting list of articles about Monuments and Buildings in the source language ''' + source_language + ''', and its availability in the target languages.

        The Qitem column provides the id and a link to the Wikidata corresponding page. The column Title provides the title in the source language. The next columns (editors, edits, pageviews, interwiki, creation date) show the value for some features in the first source language. The column Monument or building  tells the type of monument or building (in '''+source_language+''' or English if not found)  in which the algorithm identified this article as part of the Monuments and Buildings collection. The column Target Langs. provides links to the article version in each of the selected target languages. The last column shows the title in the first target language.
        '''
        ...
        if len(df) == 0:
            ...
            layout = html.Div([
                navbar,
                html.H3('Monuments and Buildings Articles', style={'textAlign':'center'}),
                html.Br(),
        ...
        print (columns)
        item_labels_dict = dash_apps_dev.get_item_labels(df['Monument or building'].drop_duplicates(keep='first'),source_lang)
...
#                    print (target_langs_titles)
                ...
                elif col == 'Monument or building':
                    label = item_labels_dict[rows['Monument or building']]
                    df_row.append(html.A( label, href='https://www.wikidata.org/wiki/'+rows['Monument or building'], target="_blank", style={'text-decoration':'none'}))
