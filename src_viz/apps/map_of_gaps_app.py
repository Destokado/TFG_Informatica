...
app_title = "Map of Geolocated Articles"
app.title = app_title + title_addenda
...
topic_dict = {'All': 'all', 'Keywords': 'keywords', 'People': 'people', 'Women': 'women',
              ...
              'Not people': 'not_people', 'CCC': 'ccc', 'CCC Not People': 'ccc_not_people', 'Medicine': 'medicine', 'LGBT':'lgbt_topic'}
...
features_dict = {'Editors': 'num_editors', 'Edits': 'num_edits', 'Pageviews': 'num_pageviews', 'Inlinks': 'num_inlinks',
                 ...
                 'Creation Date': 'date_created', 'Inlinks from CCC': 'num_inlinks_from_CCC',
...
interface_row2 = html.Div([
    ...
    html.Div(
        [
            ...
            dbc.Tooltip(
                html.P(
                    "Select a feature to sort the results and size the bubbles accordingly.",
...
def build_map_figure(df, order_by,source_language):
    df[['Lat','Lon']] = df.Geocoordinates.str.split(',',expand=True)
    df= df.drop('Geocoordinates',axis='columns')
    df['Qitem'].replace(df['Qitem'],"<a href=\"https://www.wikidata.org/wiki/{}/\">{}</a>".format(df['Qitem'],df['Qitem']))
    df[source_language+' Title'].replace(to_replace= '_', value=' ', inplace=True)

    df['Text'] = 'The article: '+df[source_language+' Title']+ ' with Qitem: '+df['Qitem']+' is geolocated at ('+df['Lat']+', '+df['Lon']+')'

    fig = px.scatter_geo(df,lat=df['Lat'],lon=df['Lon'], color=df['Target Langs.'],projection='natural earth',center={'lat':float(df['Lat'].iloc[0]),'lon':float(df['Lon'].iloc[0])},text='Text',hover_name=df[source_language+' Title'], size=order_by, title='Map of Geolocated Gaps')
    fig.update_layout(hovermode='closest', width=1280,height=720)
    return fig
...
def build_map_figure_2(df, order_by,source_language,combinations):

    df[['Lat','Lon']] = df.Geocoordinates.str.split(',',expand=True) #Split coordinates in lat and lon
    df.drop('Geocoordinates',axis='columns',inplace=True) #Delete joint column
    df[source_language+' Title'].replace(to_replace= '_', value=' ', inplace=True,regex=True)

    df['Text'] = df[source_language+' Title']+'<br>'+df['Qitem']
    df[order_by]= df[order_by].apply(pd.to_numeric) #Convert string to numeric
    df[order_by].fillna(0,inplace=True)
    fig = go.Figure()
    scale = 50
    ...
    fig.update_layout(
        title_text = 'Map of Geolocated articles <br>(Click legend to toggle availability in Target Languages)',
        showlegend= True,
        hovermode='closest',
        width=1280,
        height=720,
        geo= go.layout.Geo(
            ...
            center={'lat': float(df['Lat'].iloc[0]), 'lon': float(df['Lon'].iloc[0])},
...
def build_columns_and_query(columns, limit, order_by, source_lang, topic):
    query = 'SELECT '
...
def dash_app19_build_layout(params):
    ...
    if len(params) != 0 and params['target_langs'].lower() != 'none' and params['source_lang'].lower() != 'none':
        ...
        columns, query = build_columns_and_query(columns, limit, order_by, source_lang, topic)
        ...
        if order_by == "none" or order_by == "None":
            ...
            order_by = 'num_pageviews'
        else:
            df = df.sort_values(by=order_by, ascending=False)

        df = df.fillna('')

        columns_dict_OG = { 'page_title': source_language + ' Title', 'qitem': 'Qitem', 'geocoordinates':'Geocoordinates'}
        columns_dict = {'num': 'Nº', 'page_title': source_language + ' Title', 'target_langs': 'Target Langs.', 'qitem': 'Qitem', 'geocoordinates':'Geocoordinates'}
        columns_dict.update(features_dict_inv)

        main_title = 'List of geolocated articles retrieved from ' + source_language + ' Wikipedia and its coverage by the target languages'
        text_results_map = '''The following map shows the resulting list of articles in the source language ''' + source_language + ''', and its availability in the target languages. The size of the bubble is set by '''+columns_dict[order_by] + '''.<br>You can filter by availability in target language by interacting with the map's legend '''
        text_results = '''
        The Qitem column provides the id and a link to the Wikidata corresponding page. The column Title provides the title in the source language. The next columns (editors, edits, pageviews, interwiki, creation date) show the value for some features in the first source language. The column Geocoordinates tells the latitude and longitude coordinates with a link to visualize more precisely the location in Google Maps. The column Target Langs. provides links to the article version in each of the selected target languages. The last column shows the title in the first target language.
        '''
            ...
            layout = html.Div([
                ...
                html.Div(
                    dash_apps_dev.apply_default_value(params)(dcc.Dropdown)(
                        ...
                        value='num_pageviews',
                        placeholder="Order by ( Pageviews by default)",
                        style={'width': '190px'}
                    ), style={'display': 'inline-block', 'width': '200px'}),
        ...
        for index,rows in df.iterrows():
                ...
                if col == 'Nº':
                    ...
                    for x in range(0, len(target_langs)):
                        ...
                        if cur_title != None and cur_title != '' and cur_title != 0:
                            ...
                            text += '[' + target_langs[x] + ']' + '(' + 'http://' + target_langs[
                                x] + '.wikipedia.org/wiki/' + cur_title.replace(' ', '_') + ')'
            ...
            if show_gaps == 'one-gap-min' and z == 0:
                continue
            elif show_gaps == 'only-gaps' and z < len(target_langs):
                continue
        ...
        # RESULTS
        df_map = df[columns_dict_OG.values()]
        df_map[columns_dict[order_by]] = df[columns_dict[order_by]] #ADD the Order BY column
        langlist = []
        ...
        combinations = combs(target_langs)
        #Work only with the first n rows (n=limit)
        map_fig = build_map_figure_2(df_map.head(limit),columns_dict[order_by], source_language,combinations)
        ...
        layout = html.Div([
            ...
            html.Hr(),
            html.H5('Map'),
            dcc.Markdown(text_results_map.replace('  ', '')),
