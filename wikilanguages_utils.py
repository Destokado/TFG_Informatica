...
def load_wikipedia_language_editions_numberofarticles(wikilanguagecode, db):
    ...
    conn = sqlite3.connect(databases_path + database);
    cursor = conn.cursor()
...
def load_territories_names_from_language_country(ISO3166, languagecode, territories):
    ...
    if isinstance(territorynames, str):
        territorynames = [territorynames]
    else:
        territorynames = list(territorynames)

    return territorynames


def load_iso_3166_to_geographical_regions():
    conn = sqlite3.connect(databases_path + diversity_categories_db);
    cursor = conn.cursor();
...
def load_countries_subdivisions_from_language(languagecode, territories):
    ...
    if isinstance(subdivisions, str):
        ...
        subdivisions = list(subdivisions.unique())
    else:
        subdivisions = []
...
def load_language_pairs_apertium(wikilanguagecodes):
    ...
    try:
        ...
        for languagecode_1 in wikilanguagecodes:
            ...
            for languagecode_2 in wikilanguagecodes:
                ...
                try:
                    r = requests.post(
                        "https://cxserver.wikimedia.org/v2/translate/" + languagecode_2 + "/" + languagecode_1 + "/Apertium",
                        data={'html': '<div>' + title + '</div>'},
                        timeout=0.5)  # https://cxserver.wikimedia.org/v2/?doc  https://codepen.io/santhoshtr/pen/zjMMrG
    ...
    print('* load_language_pairs_apertium Function completed after: ' + str(
        datetime.timedelta(seconds=time.time() - functionstartTime)))
...
def load_all_countries_qitems():
    ...
    data = requests.get(url, headers={'User-Agent': 'https://wikitech.wikimedia.org/wiki/User:Marcmiquel'},
...
def get_old_current_countries_pairs(languagecode, regional):
    ...
    if languagecode == '':
        ...
        try:
            countries_language = set(territories.loc[languagecode]['ISO3166'].tolist())
        except:
            try:
                countries_language = set(); countries_language.add(territories.loc[languagecode]['ISO3166'])
            except:
                pass
        ...
        try:
            country_qitems = territories.loc[territories['regional'] == 'no'].loc[languagecode][
                'QitemTerritory'].tolist()
        except:
            try:
                country_qitems = list(); country_qitems.append(
                    territories.loc[territories['regional'] == 'no'].loc[languagecode]['QitemTerritory'])
            except:
...
def obtain_proximity_wikipedia_languages_lists(languagecode, wikipedialanguage_numberarticles, top_val, upperlower_vals,
                                               closest_val):
    # BIGGEST    # recommended 19
    top = []
    if top_val != None:
        wikipedialanguage_numberarticles_sorted = sorted(wikipedialanguage_numberarticles,
                                                         key=wikipedialanguage_numberarticles.get, reverse=True)

        top = wikipedialanguage_numberarticles_sorted[:top_val]
        if languagecode in top:
            top.append(wikipedialanguage_numberarticles_sorted[top_val + 1])
        else:
    ...
    if upperlower_vals != None:
        ...
        upperlower = wikipedialanguage_numberarticles_sorted[
                     i - upperlower_vals[0]:i] + wikipedialanguage_numberarticles_sorted[i:i + upperlower_vals[1]]
    ...
    if closest_val != None:
        ...
        cursor = conn.cursor()
        query = 'SELECT set2, rel_value FROM wcdo_intersections WHERE set1 = "' + languagecode + '" AND set1descriptor = "ccc" AND set2descriptor = "wp" AND measurement_date IN (SELECT MAX(measurement_date) FROM wcdo_intersections) ORDER BY 2 DESC LIMIT ' + str(
            closest_val)
        for row in cursor.execute(query):
...
def obtain_closest_for_all_languages(wikipedialanguage_numberarticles, wikilanguagecodes, num):
    ...
    if rows == None:
        for languagecode in wikilanguagecodes:
            top, upperlower, closest = obtain_proximity_wikipedia_languages_lists(languagecode,
                                                                                  wikipedialanguage_numberarticles,
                                                                                  None, None, num)
            if len(closest) == 0: continue
            row = ''
...
def store_group_identities_labels(wikilanguagecodes):
    functionstartTime = time.time()
    conn3 = sqlite3.connect(databases_path + diversity_categories_db);
    cursor3 = conn3.cursor()
    conn2 = sqlite3.connect(databases_path + wikipedia_diversity_db);
    cursor2 = conn2.cursor()

    cursor3.execute("DROP TABLE IF EXISTS groups_labels;")
    conn3.commit()
    cursor3.execute(
        "CREATE TABLE IF NOT EXISTS groups_labels (qitem text, label integer, lang text, group_label text, PRIMARY KEY (qitem, label, lang, group_label));")
    conn3.commit()
...
def store_lgbt_label(store_get):
    functionstartTime = time.time()
    conn = sqlite3.connect(databases_path + wikipedia_diversity_production_db);
    cursor = conn.cursor()
    conn2 = sqlite3.connect(databases_path + diversity_categories_db);
    cursor2 = conn2.cursor()
...
def establish_mysql_connection_read(languagecode):
    #    print (languagecode)
    try:
        mysql_con_read = mdb.connect(host=languagecode + "wiki.analytics.db.svc.eqiad.wmflabs",
                                     db=languagecode + 'wiki_p', read_default_file=os.path.expanduser("./my.cnf"),
...
def establish_mysql_connection_write():
    mysql_con_write = mdb.connect(host="tools.db.svc.eqiad.wmflabs", db='s53619__wcdo',
                                  read_default_file=os.path.expanduser("./my.cnf"), charset='utf8mb4')
...
#     ?item wdt:P1800 ?bbddwp.
    ...
    for item in data['results']['bindings']:
        ...
        if wikimedialanguagecode != item['WikimediaLanguagecode']['value'] and wikimedialanguagecode != '':
            ...
            try:
                currentLanguagename = Locale.parse(wikimedialanguagecode).get_display_name(
                    wikimedialanguagecode).lower()
        ...
        try:
            englisharticle = item['languagelink']['value']
        except:
            englisharticle = 'no link'
...
def extract_wikipedia_diversity_tables_to_files():
    for languagecode in wikilanguagecodes:
        ...
        # These are the files.
        ccc_filename_archived = languagecode + 'wiki_' + str(
            cycle_year_month.replace('-', '')) + '_diversity.csv'  # (e.g. 'cawiki_20180215_ccc.csv')
        ...
        cursor.execute(
            "SELECT * FROM " + languagecode + "wiki;")  # ->>>>>>> canviar * per les columnes. les de rellevància potser no cal.
...
def get_new_cycle_year_month():
    ...
    current_cycle_date = datetime.datetime.strptime(current_cycle_date,
                                                    '%Y%m%d%H%M%S') - dateutil.relativedelta.relativedelta(months=1)
...
def get_last_accumulated_period_year_month():
    ...
    conn = sqlite3.connect(databases_path + stats_production_db);
    cursor = conn.cursor()
...
def copy_db_for_production(dbname, scriptname, databases_path):
    ...
    try:
        ...
        duration = str(datetime.timedelta(seconds=time.time() - functionstartTime))
        print(
            'File ' + dbname + ' copied as ' + dbname_production + ' at the end of the ' + scriptname + ' script. It took: ' + duration)
    except:
...
def finish_email(startTime, filename, title):
    try:
        sys.stdout = None;
        send_email_toolaccount(title + ' completed successfuly', open(filename, 'r').read())
    except Exception as err:
        print('* Task aborted after: ' + str(datetime.timedelta(seconds=time.time() - startTime)))
        sys.stdout = None;
        send_email_toolaccount(title + ' aborted because of an error', open(filename, 'r').read() + 'err')
...
def backup_db():
    try:
        ...
        print(
            'File wikipedia_diversity.db copied as wikipedia_diversity_backup.db at the end of the content_retrieval.py script.')
    except:
...
def verify_function_run(cycle_year_month, script_name, function_name, action, duration):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS function_account (script_name text, function_name text, year_month text, finish_time text, duration text, PRIMARY KEY (script_name, function_name, year_month));")
    ...
    if action == 'check':
        ...
        if duration != None:
            print(
                '= Process Accountant: The function "' + function_name_string + '" has already been run. It lasted: ' +
                duration[0])
            return 1
        else:
            print(
                '- Process Accountant: The function "' + function_name_string + '" has not run yet. Do it! Now: ' + str(
                    datetime.datetime.utcnow().strftime(
                        "%Y/%m/%d-%H:%M:%S") + '. Year Month Cycle: ' + cycle_year_month))
            return 0
    ...
    if action == 'mark':
        ...
        conn.commit()
        print(
            '+ Process Accountant. Function: ' + function_name + ' is NOW RUN! Script: ' + script_name + '. After ' + duration + '.\n')
...
def verify_script_run(cycle_year_month, script_name, action, duration):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS script_account (script_name text, year_month text, finish_time text, duration text, PRIMARY KEY (script_name, year_month));")
    ...
    if action == 'check':
        ...
        duration = cursor.fetchone()
        if duration != None:
            print('= Process Accountant: The script "' + script_name_string + '" has already been run. It lasted: ' +
                  duration[0])
            return 1
        else:
            print('- Process Accountant: The script "' + script_name_string + '" has not run yet. Do it! Now: ' + str(
                datetime.datetime.utcnow().strftime("%Y/%m/%d-%H:%M:%S") + '. Year Month Cycle: ' + cycle_year_month))
            return 0
...
def check_time_for_script_run(script_name, cycle_year_month):
    ...
    while (not_ready):
        ...
        print("Let's check if it is time for " + script_name + ' to run: ' + str(
            datetime.datetime.utcnow().strftime("%Y/%m/%d-%H:%M:%S")) + '. Cycle: ' + cycle_year_month)
    ...
    print('* It is time for ' + script_name + ' to run: ' + str(
        datetime.datetime.utcnow().strftime("%Y/%m/%d-%H:%M:%S")) + '. Cycle: ' + cycle_year_month)
...
def store_lines_per_second(duration, lines, function_name, file, period):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS lines_per_second (linespersecond real, lines integer, duration integer, function_name text, file text, year_month text, PRIMARY KEY (function_name, year_month));")
    ...
    print('in function ' + function_name + ' reading the dump ' + file + ', the speed is ' + str(
        linespersecond) + ' lines/second, at this period: ' + period)
...
def check_run_failed_functions():
    ...
    function_parameters = [{'label_ccc_articles_geolocation_wd': (languagecode, page_titles_page_ids),
                            'label_ccc_articles_geolocated_reverse_geocoding': (
                            languagecode, page_titles_qitems, page_titles_page_ids),
                            ...
                            'label_ccc_articles_keywords': (languagecode, page_titles_qitems)}, {
                               'label_potential_ccc_articles_category_crawling': (
                               languagecode, page_titles_page_ids, page_titles_qitems),
                               'label_potential_ccc_articles_with_inlinks ccc': (
                               languagecode, page_titles_page_ids, page_titles_qitems, 'ccc'),
                               'label_potencial_ccc_articles_with_outlinks ccc': (
                               languagecode, page_titles_page_ids, page_titles_qitems, 'ccc'),
                               'label_potential_ccc_articles_language_weak_wd': (languagecode, page_titles_page_ids),
                               'label_potential_ccc_articles_affiliation_properties_wd': (
                               languagecode, page_titles_page_ids),
                               'label_potential_ccc_articles_has_part_properties_wd': (
                               languagecode, page_titles_page_ids)}, {'label_other_ccc_wikidata_properties': (
    languagecode, page_titles_page_ids, page_titles_qitems), 'label_potential_ccc_articles_with_inlinks no_ccc': (
    languagecode, page_titles_qitems, 'no ccc'), 'label_potencial_ccc_articles_with_outlinks no_ccc': (
    languagecode, page_titles_page_ids, 'no ccc')}, {'calculate_articles_ccc_binary_classifier': (
    languagecode, 'RandomForest', page_titles_page_ids, page_titles_qitems),
    ...
    for languagecode in wikilanguagecodes:
        for functions in function_parameters:
            for function, parameters in functions.items():
                ...
                if num == 0:
                    (page_titles_qitems, page_titles_page_ids) = wikilanguages_utils.load_dicts_page_ids_qitems(0,
                                                                                                                languagecode)
                    exec(function, parameters)


def check_language_article_features():
    features = ['num_inlinks', 'num_outlinks', 'num_bytes', 'num_references', 'num_edits', 'num_editors',
                'num_discussions', 'num_pageviews', 'num_wdproperty', 'num_interwiki', 'num_images',
                'num_edits_last_month', 'featured_article', 'wikirank']
...
def copy_language_article_features():
    ...
    conn = sqlite3.connect(databases_path + 'ccc.db');
    cursor = conn.cursor()
...
def clean_failed_function_account():
    ...
    function_parameters = {'label_diversity_categories_related_topics_wd': 'gender',
                           'label_ccc_articles_geolocation_wd': 'geocoordinates',
                           'label_ccc_articles_geolocated_reverse_geocoding': 'ccc_geolocated',
                           'label_ccc_articles_country_wd': 'country_wd',
                           'label_ccc_articles_location_wd': 'location_wd',
                           'label_ccc_articles_language_strong_wd': 'language_strong_wd',
                           'label_ccc_articles_created_by_properties_wd': 'created_by_wd',
                           'label_ccc_articles_part_of_properties_wd': 'part_of_wd',
                           'label_ccc_articles_keywords': 'keyword_title',
                           'label_potential_ccc_articles_category_crawling': 'category_crawling_level',
                           'label_potential_ccc_articles_with_inlinks ccc': 'num_inlinks',
                           'label_potencial_ccc_articles_with_outlinks ccc': 'num_outlinks',
                           'label_potential_ccc_articles_language_weak_wd': 'language_weak_wd',
                           'label_potential_ccc_articles_affiliation_properties_wd': 'affiliation_wd',
                           'label_potential_ccc_articles_has_part_properties_wd': 'has_part_wd',
                           'label_other_ccc_wikidata_properties': 'other_ccc_language_strong_wd',
                           'label_other_ccc_category_crawling': 'other_ccc_category_crawling_relative_level',
                           'label_potential_ccc_articles_with_inlinks no_ccc': 'num_inlinks_from_geolocated_abroad',
                           'label_potencial_ccc_articles_with_outlinks no_ccc': 'num_outlinks_to_geolocated_abroad',
                           'calculate_articles_ccc_main_territory': 'main_territory',
                           'calculate_articles_ccc_retrieval_strategies': 'num_retrieval_strategies'}
    ...
    for languagecode in wikilanguagecodes:
        ...
        for function, parameter in function_parameters.items():
            ...
            query = 'SELECT count(*) FROM function_account WHERE function_name = "' + function + ' ' + languagecode
            if function == 'label_diversity_categories_related_topics_wd':
                query += ' gender"'
            else:
                query += '"'
            ...
            if num2 != 0 and num == 0:
                print(
                    function + ' ' + languagecode + ' exists and the database is empty. we delete function account so the function runs again next time.')
            ...
            if num != 0 and num2 == 0:
                print(
                    parameter + ' ' + languagecode + ' is in the database but the function ' + function + ' ' + languagecode + ' has not been accounted.')
        conn.commit()

    conn = sqlite3.connect(databases_path + wikipedia_diversity_db);
    cursor = conn.cursor()

    function_parameters = {'extend_articles_timestamp': 'date_created', 'extend_articles_editors': 'num_editors',
                           'extend_articles_discussions': 'num_discussions', 'extend_article_edits': 'num_edits',
                           'extend_articles_edits_last_month': 'num_edits_last_month',
                           'extend_articles_references': 'num_references', 'extend_articles_bytes': 'num_bytes',
                           'extend_articles_interwiki': 'num_interwiki',
                           'extend_articles_qitems_properties': 'num_wdproperty',
                           'extend_articles_featured': 'featured_article', 'extend_articles_images': 'num_images',
                           'extend_articles_pageviews': 'num_pageviews'}
    ...
    for languagecode in wikilanguagecodes:
        ...
        for function, parameter in function_parameters.items():
            ...
            query = 'SELECT count(*) FROM function_account WHERE function_name = "' + function + ' ' + languagecode
            if function == 'extend_articles_wikidata_topics':
                query += ' gender"'
            else:
                query += '"'
