...
### DASH APP ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ###
app = Dash(__name__, server=app_dev, url_base_pathname=webtype + '/medicine_articles/',
...
topic_dict = {'All': 'all', 'Keywords': 'keywords', 'Geolocated': 'geolocated', 'People': 'people', 'Women': 'women',
              'Men': 'men', 'Folk': 'folk', 'Earth': 'earth', 'Monuments and Buildings': 'medicine',
              ...
              'Not people': 'not_people', 'CCC': 'ccc', 'CCC Not People': 'ccc_not_people'}
...
def dash_app19_build_layout(params):
    ...
    if len(params) != 0 and params['target_langs'].lower() != 'none' and params['source_lang'].lower() != 'none':
        ...
        if order_by == "none" or order_by == "None":
            df = df.sort_values(by='medicine', ascending=False)
        else:
        ...
        main_title = 'Medicine articles retrieved from ' + source_language + ' Wikipedia and its coverage by the target languages'
        ...
        for index, rows in df.iterrows():
            ...
            for col in columns:
                if col == 'Nº':
                    ...
                    df_row.append(
                        html.A(rows['Medicine Topic'], href='https://www.wikidata.org/wiki/' + rows['Medicine Topic'],
                               target="_blank",
                               style={'text-decoration': 'none'}))
                    # df_row.append(rows['Medicine Topic'])
