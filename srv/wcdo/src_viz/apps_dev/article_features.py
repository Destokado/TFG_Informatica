...
def extend_articles_interwiki_qitem_properties_identifiers_sister_projects(languagecode, page_titles_page_ids):
    ...
    updated = []
    query = "SELECT metadata.qitem, metadata.sitelinks, metadata.properties, metadata.wd_identifiers, metadata.sisterprojects_sitelinks FROM metadata INNER JOIN sitelinks ON sitelinks.qitem = metadata.qitem WHERE sitelinks.langcode = '"+languagecode+"wiki'"
...
def extend_articles_history_features(languagecode,page_titles_qitems, page_titles_page_ids):
                ...
                if user_id != "":
                    try:
                        ...
                        try: 
                            ...
                            editor_history[user_id] = (user_id, user_flag, year_first_edit)
    ...
    conn = sqlite3.connect(databases_path + wikipedia_diversity_db); cursor = conn.cursor()
    conn2 = sqlite3.connect(databases_path + 'editors_pages.db'); cursor2 = conn2.cursor()
    query = 'SELECT count(*), page_id FROM editors INNER JOIN editors_history ON editors.editor = editors_history.editor WHERE editors_history.flag IN ("sysop","bureaucrat","oversight","checkuser","steward") GROUP BY page_id ORDER BY page_id;'
    for row in cursor2.execute(query):
        page_id = row[1]
        try:
            parameters.append((row[0],page_id,page_ids_qitems[page_id]))
