...
### DASH APP ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ###
dash_app30 = Dash(server = app_dev, url_base_pathname = webtype + '/content_gaps_metrics/', external_stylesheets = external_stylesheets)
dash_app30.config['suppress_callback_exceptions']=True

dash_app30.title = 'Content Gaps Metrics'+title_addenda
dash_app30.layout = html.Div([
...
def dash_app30_build_layout(params):
...
@dash_app30.callback(Output('url', 'search'),
...
@dash_app30.callback(Output('page-content', 'children'),
              inputs=[Input('url', 'href')])
def page_load(href):
    if not href:
        return []
    state = dash_apps_dev.parse_state(href)
    return dash_app30_build_layout(state)
