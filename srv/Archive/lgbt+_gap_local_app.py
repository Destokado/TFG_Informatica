...
# langs = ['ca','es','it','ro','en']
# content_type = 'lgbt_topics_coverage_extent'
# df = params_to_df(langs, content_type)
# print (df.head(10))
# input('')

# SELECT set1,set1descriptor,set2,set2descriptor, abs_value, rel_value, period FROM wdo_intersections_accumulated WHERE content = 'articles' AND set1 IN ('ceb', 'nl', 'en', 'fr', 'de', 'it', 'pl', 'ru', 'es', 'sv') AND set1descriptor = 'people' AND set2 = 'sexual_orientation' AND period = '2020-09' ORDER BY abs_value DESC;
...
if __name__ == '__main__':
    dash_app15.run_server(debug=True)#,dev_tools_ui=False)
